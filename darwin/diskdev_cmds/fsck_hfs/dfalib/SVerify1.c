...
#define kIDSector 2
	...
	if (SWAP_BE16(mdbp->drSigWord) == kHFSPlusSigWord) {
		...
		if (SWAP_BE16(mdbp->drEmbedSigWord) == kHFSPlusSigWord) {
			...
			if (markClean && (result == 0)) {
				mdbp->drAtrb |= SWAP_BE16(kHFSVolumeUnmountedMask);
	...
	if ((vhp != NULL) && (ValidVolumeHeader(vhp) == noErr)) {
		...
		if (markClean && (result == 0)) {
			vhp->attributes |= SWAP_BE32(kHFSVolumeUnmountedMask);
			rbOptions = kForceWriteBlock;
		}
	}
...
OSErr	ScavengeVolumeType( SGlobPtr GPtr, HFSMasterDirectoryBlock *mdb, UInt16 *volumeType  )
{
	...
	*volumeType	= kEmbededHFSPlusVolumeType;		//	Assume HFS+
	...
	if ( embedSigWord != kHFSPlusSigWord )
	{
		...
		embededExtent.startBlock	= (vHSector - 2 - SWAP_BE16(mdb->drAlBlSt) ) / sectorsPerBlock;
		embedSigWord				= kHFSPlusSigWord;
	}
	else
	{
		embedSigWord				= SWAP_BE16(mdb->drEmbedSigWord);
		embededExtent.blockCount	= SWAP_BE16(mdb->drEmbedExtent.blockCount);
		embededExtent.startBlock	= SWAP_BE16(mdb->drEmbedExtent.startBlock);
	}
		
	if ( embedSigWord == kHFSPlusSigWord )
	{
		startSector = 2 + SWAP_BE16(mdb->drAlBlSt) +
			((UInt64)embededExtent.startBlock * (SWAP_BE32(mdb->drAlBlkSiz) / 512));
			
		err = SeekVolumeHeader( GPtr, startSector, SWAP_BE32(mdb->drAlBlkSiz) / 512, &vHSector );
		if ( err != noErr ) goto AssumeHFS;
	
		//	Now replace the bad fields and mark the error
		mdb->drEmbedExtent.blockCount	= SWAP_BE16(hfsPlusSectors / sectorsPerBlock);
		mdb->drEmbedExtent.startBlock	= SWAP_BE16((vHSector - 2 - SWAP_BE16(mdb->drAlBlSt) ) / sectorsPerBlock);
		mdb->drEmbedSigWord				= SWAP_BE16(kHFSPlusSigWord);
		mdb->drAlBlSt					+= SWAP_BE16(vHSector - startSector);								//	Fix the bad field
...
static OSErr CheckWrapperExtents( SGlobPtr GPtr, HFSMasterDirectoryBlock *mdb )
{
	...
	if ( SWAP_BE16(mdb->drCTExtRec[0].startBlock) >= SWAP_BE16(mdb->drEmbedExtent.startBlock) )
	{
		...
		mdb->drCTExtRec[0].startBlock = SWAP_BE16(SWAP_BE16(mdb->drXTExtRec[0].startBlock) + SWAP_BE16(mdb->drXTExtRec[0].blockCount));
		GPtr->VIStat = GPtr->VIStat | S_MDB;													//	write out our MDB
...
------------------------------------------------------------------------------*/
#define GetRecordOffset(btreePtr,node,index)		SWAP_BE16(*(short *) ((UInt8 *)(node) + (btreePtr)->nodeSize - ((index) << 1) - kOffsetSize))
static	OSErr	CheckNodesFirstOffset( SGlobPtr GPtr, BTreeControlBlock *btcb )
{
	NodeRec		nodeRec;
	UInt16		offset;
...
OSErr CatHChk( SGlobPtr GPtr )
{
	...
	while ( (GPtr->DirLevel > 0) && (result == noErr) )
	{
		...
		//	 if same ParID ...
		//
		parID = SWAP_BE32(isHFSPlus == true ? foundKey.hfsPlus.parentID : foundKey.hfs.parentID);
				...
				if ( result != noErr ) {
					...
					for (mtp = GPtr->missingThreadList; mtp != NULL; mtp = mtp->link) {
						if (mtp->threadID == dprP->directoryID) {
							mtp->thread.recordType = SWAP_BE16(kHFSPlusFolderThreadRecord);
							mtp->thread.parentID = SWAP_BE32(dprP->parentDirID);
...
static	OSErr	RcdMDBEmbededVolDescriptionErr( SGlobPtr GPtr, OSErr type, HFSMasterDirectoryBlock *mdb )
{
	...
	desc->drEmbedExtent.startBlock	=  SWAP_BE16(mdb->drEmbedExtent.startBlock);
	desc->drEmbedExtent.blockCount	=  SWAP_BE16(mdb->drEmbedExtent.blockCount);
...
OSErr VInfoChk( SGlobPtr GPtr )
{
	...
	else		//	HFS
	{
		...
		//	just copy cache parameters for now
	//	vcb->vcbEmbedSigWord			= SWAP_BE16(alternateMDB->drEmbedSigWord);
	//	vcb->vcbEmbedExtent.startBlock	= SWAP_BE16(alternateMDB->drEmbedExtent.startBlock);
	//	vcb->vcbEmbedExtent.blockCount	= SWAP_BE16(alternateMDB->drEmbedExtent.blockCount);
...
OSErr	VLockedChk( SGlobPtr GPtr )
{
	...
	{
		frFlags = SWAP_BE16(SWAP_BE16(record.recordType) == kHFSPlusFolderRecord ? record.hfsPlusFolder.userInfo.frFlags : record.hfsFolder.userInfo.frFlags);
...
void	BuildExtentKey( Boolean isHFSPlus, UInt8 forkType, HFSCatalogNodeID fileNumber, UInt32 blockNumber, void * key )
{
	if ( isHFSPlus )
	{
		...
		hfsPlusKey->keyLength	= SWAP_BE16(kHFSPlusExtentKeyMaximumLength);
		hfsPlusKey->forkType	= forkType;
		hfsPlusKey->pad			= 0;
		hfsPlusKey->fileID		= SWAP_BE32(fileNumber);
		hfsPlusKey->startBlock	= SWAP_BE32(blockNumber);
	}
	else
	{
		...
		hfsKey->fileID			= SWAP_BE32(fileNumber);
		hfsKey->startBlock		= SWAP_BE16((UInt16) blockNumber);
	}
}
