...
OSStatus	UpdateHeader	(BTreeControlBlockPtr		btreePtr)
{
	...
	header->treeDepth		= SWAP_BE16(btreePtr->treeDepth);
	header->rootNode		= SWAP_BE32(btreePtr->rootNode);
	header->leafRecords		= SWAP_BE32(btreePtr->leafRecords);
	header->firstLeafNode	= SWAP_BE32(btreePtr->firstLeafNode);
	header->lastLeafNode	= SWAP_BE32(btreePtr->lastLeafNode);
	header->nodeSize		= SWAP_BE16(btreePtr->nodeSize);			//ее this shouldn't change
	header->maxKeyLength	= SWAP_BE16(btreePtr->maxKeyLength);		//ее neither should this
	header->totalNodes		= SWAP_BE32(btreePtr->totalNodes);
	header->freeNodes		= SWAP_BE32(btreePtr->freeNodes);
