...
OSStatus	AllocateNode (BTreeControlBlockPtr		btreePtr, UInt32	*nodeNum)
{
	...
	//////////////////////// Find Word with Free Bit ////////////////////////////
		...
		while ( size-- )
		{
			if ( SWAP_BE16(*pos++) != 0xFFFF )			// assume test fails, and increment pos
				break;
		}

		--pos;								// whoa! backup

		if (SWAP_BE16(*pos) != 0xFFFF)					// hey, we got one!
			break;
...
OSStatus	FreeNode (BTreeControlBlockPtr		btreePtr, UInt32	nodeNum)
{
	...
	*mapPos &= SWAP_BE16(~(1 << bitOffset));			// clear it
						 ...
						 UInt32					newTotalNodes )
{
	...
	((BTNodeDescriptor*)mapNode.buffer)->fLink = SWAP_BE32(firstNewMapNodeNum);
	...
	while (true)
	{
		...
		((NodeDescPtr)newNode.buffer)->numRecords	= SWAP_BE16(1);
		((NodeDescPtr)newNode.buffer)->kind			= kBTMapNode;
		
		// set free space offset
		*(UInt16 *)((Ptr)newNode.buffer + nodeSize - 4) = SWAP_BE16(nodeSize - 6);

		if (nodeNum++ == lastNewMapNodeNum)
			break;

		((BTNodeDescriptor*)newNode.buffer)->fLink = SWAP_BE32(nodeNum);	// point to next map node
