								 ...
								 KeyDescriptorPtr		 keyDescPtr )
{
	...
	header->node.numRecords		= SWAP_BE16(3);			// header rec, key desc, map rec

	header->nodeSize			= SWAP_BE16(nodeSize);
	header->maxKeyLength		= SWAP_BE16(maxKeyLength);
	header->btreeType			= btreeType;
	header->totalNodes			= SWAP_BE32(btreePtr->totalNodes);
	header->freeNodes			= SWAP_BE32(btreePtr->totalNodes - 1);
	...
	*pos	= 0x80;
	...
	pos	 = SWAP_BE16(((Ptr)headerNode.buffer) + nodeSize);
	pos	-= 2;		*((UInt16 *)pos) = SWAP_BE16(kHeaderRecOffset);
	pos	-= 2;		*((UInt16 *)pos) = SWAP_BE16(kKeyDescRecOffset);
	pos -= 2;		*((UInt16 *)pos) = SWAP_BE16(kHeaderMapRecOffset);
	pos -= 2;		*((UInt16 *)pos) = SWAP_BE16(nodeSize - 8);
