...
SInt32 CompareAttributeKeys( const void *inSearchKey, const void *inTrialKey )
{
	 ...
	 */
	if (SWAP_BE32(searchKey->cnid) != SWAP_BE32(trialKey->cnid)) {
		return (SWAP_BE32(searchKey->cnid) < SWAP_BE32(trialKey->cnid) ? -1 : 1);
	}
	...
	if (SWAP_BE32(searchKey->startBlock) == SWAP_BE32(trialKey->startBlock))
		...
		return (SWAP_BE32(searchKey->startBlock) < SWAP_BE32(trialKey->startBlock) ? -1 : 1);
}
