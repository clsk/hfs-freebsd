...
BuildCatalogKey(HFSCatalogNodeID parentID, const CatalogName *cName, Boolean isHFSPlus, CatalogKey *key)
{
	if ( isHFSPlus )
	{
		key->hfsPlus.keyLength		= SWAP_BE16(kHFSPlusCatalogKeyMinimumLength);	// initial key length (4 + 2)
		key->hfsPlus.parentID			= SWAP_BE32(parentID);		// set parent ID
		key->hfsPlus.nodeName.length	= SWAP_BE16(0);			// null CName length
		if ( cName != NULL )
		{
			CopyCatalogName(cName, (CatalogName *) &key->hfsPlus.nodeName, isHFSPlus);
			key->hfsPlus.keyLength = SWAP_BE16(SWAP_BE16(key->hfsPlus.keyLength) + sizeof(UniChar) * SWAP_BE16(cName->ustr.length));	// add CName size to key length
		}
	}
	else
	{
		...
		key->hfs.parentID		= SWAP_BE32(parentID);			// set parent ID
...
OSErr	ValidVolumeHeader( HFSPlusVolumeHeader *volumeHeader )
{
	...
	if ( SWAP_BE16(volumeHeader->signature) == kHFSPlusSigWord &&
		 SWAP_BE16(volumeHeader->version) == kHFSPlusVersion )
	{
						...
						UInt32 attributes, UInt32 *mapNodes, void *buffer)
{
	...
	*mapNodes = 0;							// number of map nodes initially (0)
	...
	ndp->numRecords = SWAP_BE16(3);			// there are 3 records (header, map, and user)
	...
	if (nodeCount > nodeBitsInHeader)		// do we need additional map nodes?
	{
		...
		if (recordCount > 0)			// catalog B-tree?
			ndp->fLink = SWAP_BE32(2);			// link points to initial map node
											//¥¥ Assumes all records will fit in one node.  It would be better
											//¥¥ to put the map node(s) first, then the records.
		else
			ndp->fLink = SWAP_BE32(1);			// link points to initial map node
...
OSErr	FlushVolumeControlBlock( SVCB *vcb )
{
	...
	{		
		...
		if ( vcb->vcbEmbeddedOffset != 0 )  // It's a wrapped HFS+ volume
		{
			...
			if ( err == noErr )
			{
				...
				{
					mdb->drCrDate = SWAP_BE32(vcb->vcbCreateDate);
		...
		volumeHeader->attributes		= SWAP_BE32(vcb->vcbAttributes);
		volumeHeader->lastMountedVersion = SWAP_BE32(kFSCKMountVersion);
		volumeHeader->createDate		= SWAP_BE32(vcb->vcbCreateDate);  // NOTE: local time, not GMT!
		volumeHeader->modifyDate		= SWAP_BE32(vcb->vcbModifyDate);
		volumeHeader->backupDate		= SWAP_BE32(vcb->vcbBackupDate);
		volumeHeader->checkedDate		= SWAP_BE32(vcb->vcbCheckedDate);
		volumeHeader->fileCount			= SWAP_BE32(vcb->vcbFileCount);
		volumeHeader->folderCount		= SWAP_BE32(vcb->vcbFolderCount);
		volumeHeader->blockSize			= SWAP_BE32(vcb->vcbBlockSize);
		volumeHeader->totalBlocks		= SWAP_BE32(vcb->vcbTotalBlocks);
		volumeHeader->freeBlocks		= SWAP_BE32(vcb->vcbFreeBlocks);
		volumeHeader->nextAllocation		= SWAP_BE32(vcb->vcbNextAllocation);
		volumeHeader->rsrcClumpSize		= SWAP_BE32(vcb->vcbRsrcClumpSize);
		volumeHeader->dataClumpSize		= SWAP_BE32(vcb->vcbDataClumpSize);
		volumeHeader->nextCatalogID		= SWAP_BE32(vcb->vcbNextCatalogID);
		volumeHeader->writeCount		= SWAP_BE32(vcb->vcbWriteCount);
		volumeHeader->encodingsBitmap		= SWAP_BE64(vcb->vcbEncodingsBitmap);

		//¥¥Êshould we use the vcb or fcb clumpSize values ????? -djb
		volumeHeader->allocationFile.clumpSize	= SWAP_BE32(vcb->vcbAllocationFile->fcbClumpSize);
		volumeHeader->extentsFile.clumpSize	= SWAP_BE32(vcb->vcbExtentsFile->fcbClumpSize);
		volumeHeader->catalogFile.clumpSize	= SWAP_BE32(vcb->vcbCatalogFile->fcbClumpSize);
		...
		volumeHeader->extentsFile.logicalSize = SWAP_BE64(fcb->fcbLogicalSize);
		volumeHeader->extentsFile.totalBlocks = SWAP_BE32(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
	
		fcb = vcb->vcbCatalogFile;
		CopyMemory( fcb->fcbExtents32, volumeHeader->catalogFile.extents, sizeof(HFSPlusExtentRecord) );
		volumeHeader->catalogFile.logicalSize = SWAP_BE64(fcb->fcbLogicalSize);
		volumeHeader->catalogFile.totalBlocks = SWAP_BE32(fcb->fcbPhysicalSize / vcb->vcbBlockSize);

		fcb = vcb->vcbAllocationFile;
		CopyMemory( fcb->fcbExtents32, volumeHeader->allocationFile.extents, sizeof(HFSPlusExtentRecord) );
		volumeHeader->allocationFile.logicalSize = SWAP_BE64(fcb->fcbLogicalSize);
		volumeHeader->allocationFile.totalBlocks = SWAP_BE32(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		...
		if (vcb->vcbAttributesFile != NULL)	//	Only update fields if an attributes file existed and was open
		{
			...
			volumeHeader->attributesFile.logicalSize = SWAP_BE64(fcb->fcbLogicalSize);
			volumeHeader->attributesFile.clumpSize = SWAP_BE32(fcb->fcbClumpSize);
			volumeHeader->attributesFile.totalBlocks = SWAP_BE32(fcb->fcbPhysicalSize / vcb->vcbBlockSize);
		}
	...
	else
	{
		...
		mdbP->drCrDate    = SWAP_BE32(vcb->vcbCreateDate);
		mdbP->drLsMod     = SWAP_BE32(vcb->vcbModifyDate);
		mdbP->drAtrb      = SWAP_BE16((UInt16)vcb->vcbAttributes);
		mdbP->drClpSiz    = SWAP_BE32(vcb->vcbDataClumpSize);
		mdbP->drNxtCNID   = SWAP_BE32(vcb->vcbNextCatalogID);
		mdbP->drFreeBks   = SWAP_BE16(vcb->vcbFreeBlocks);
		mdbP->drXTClpSiz  = SWAP_BE32(vcb->vcbExtentsFile->fcbClumpSize);
		mdbP->drCTClpSiz  = SWAP_BE32(vcb->vcbCatalogFile->fcbClumpSize);

		mdbP->drNmFls     = SWAP_BE16(vcb->vcbNmFls);
		mdbP->drNmRtDirs  = SWAP_BE16(vcb->vcbNmRtDirs);
		mdbP->drFilCnt    = SWAP_BE32(vcb->vcbFileCount);
		mdbP->drDirCnt    = SWAP_BE32(vcb->vcbFolderCount);
...
ConvertToHFSPlusExtent( const HFSExtentRecord oldExtents, HFSPlusExtentRecord newExtents)
{
	...
	{
		newExtents[i].blockCount = SWAP_BE32(0);
		newExtents[i].startBlock = SWAP_BE32(0);
	}

	newExtents[2].blockCount = SWAP_BE32(SWAP_BE16(oldExtents[2].blockCount));
	newExtents[2].startBlock = SWAP_BE32(SWAP_BE16(oldExtents[2].startBlock));
	newExtents[1].blockCount = SWAP_BE32(SWAP_BE16(oldExtents[1].blockCount));
	newExtents[1].startBlock = SWAP_BE32(SWAP_BE16(oldExtents[1].startBlock));
	newExtents[0].blockCount = SWAP_BE32(SWAP_BE16(oldExtents[0].blockCount));
	newExtents[0].startBlock = SWAP_BE32(SWAP_BE16(oldExtents[0].startBlock));
}
