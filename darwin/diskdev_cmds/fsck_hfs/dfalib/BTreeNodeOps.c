						 ...
						 NodeRec				*returnNodePtr )
{
	...
	*(UInt16 *)pos = SWAP_BE16(sizeof (BTNodeDescriptor));		// set offset to beginning of free space
								 ...
								 UInt16					 recSize )
{
	...
	if ( btreePtr->attributes & kBTBigKeysMask )
	{
		*((UInt16*) dst)++ = SWAP_BE16(keyLength);		// use keyLength rather than key.length
								 ...
								 UInt16					 delta )
{
	...
	node->numRecords = SWAP_BE16(SWAP_BE16(node->numRecords) + 1);
	
	do {
		*dst = SWAP_BE16(SWAP_BE16(*src) + delta);
		src++;
		dst++;
	} while (numOffsets--);
}
								 ...
								 UInt16					 index )
{
	...
	while (numOffsets--)
	{
		--src;
		--dst;
		*dst = SWAP_BE16(SWAP_BE16(*src) - delta);				// work our way left
	}
}
