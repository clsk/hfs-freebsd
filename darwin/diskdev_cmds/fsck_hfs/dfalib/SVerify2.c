...
BTCheck(SGlobPtr GPtr, short refNum, CheckLeafRecordProcPtr checkLeafRecord)
{
...
#if 0
	printf( "\nB-Tree header rec: \n" );
	printf( "    treeDepth     = %d \n", SWAP_BE16(header->treeDepth) );
	printf( "    rootNode      = %d \n", SWAP_BE32(header->rootNode) );
	printf( "    leafRecords   = %d \n", SWAP_BE32(header->leafRecords) );
	printf( "    firstLeafNode = %d \n", SWAP_BE32(header->firstLeafNode) );
	printf( "    lastLeafNode  = %d \n", SWAP_BE32(header->lastLeafNode) );
	printf( "    totalNodes    = %d \n", SWAP_BE32(header->totalNodes) );
	printf( "    freeNodes     = %d \n", SWAP_BE32(header->freeNodes) );
#endif
	...
	while ( GPtr->BTLevel > 0 )
	{
		...
		if ( nodeDescP->kind == kBTIndexNode )
		{
			...
			nodeNum = SWAP_BE32(*(UInt32*)dataPtr);
...
ContinueChecking:
	...
	//  (SWAP_BE16(mdbP->drEmbedSigWord)		!= vcb->vcbEmbedSigWord)		||
	//  (SWAP_BE16(mdbP->drEmbedExtent.startBlock)	!= vcb->vcbEmbedExtent.startBlock)	||
	//  (SWAP_BE16(mdbP->drEmbedExtent.blockCount)	!= vcb->vcbEmbedExtent.blockCount)
	   )
	{ 
		GPtr->VIStat = GPtr->VIStat | S_MDB;
