...
//          Panic("\pSearchTree: curNodeNum is zero!");
		...
		curNodeNum = SWAP_BE32(*(UInt32 *)dataPtr);
						 ...
						 UInt32						*insertNode )
{
	...
	if ( insertParent || updateParent )
	{
		...
		if ( updateParent )
		{
			...
			PanicIf( SWAP_BE32(*(UInt32 *) recPtr) != targetNodeNum, "\p InsertLevel: parent ptr doesn't match target node!");
			...
			targetNodeNum = SWAP_BE32(targetNodeNum);
							 ...
							 Boolean				*rootSplit )
{
	...
	*rootSplit = false;
	...
	if ( leftNodeNum > 0 )
	{
		...
		PanicIf ( SWAP_BE32(((NodeDescPtr) siblingNode->buffer)->fLink) != nodeNum, "\p InsertNode, RotateLeft: invalid sibling link!" );
								 ...
								 UInt16						 level )
{
	...
	if ( updateRequired || deleteRequired )
	{
		...
		if ( updateRequired )
		{
			...
			//ее╩debug: check if ptr == targetNodeNum
			GetRecordByIndex (btreePtr, parentNode.buffer, index, &keyPtr, &recPtr, &recSize);
			PanicIf( SWAP_BE32(*(UInt32 *) recPtr) != targetNodeNum, "\p DeleteTree: parent ptr doesn't match targetNodeNum!!");
								 ...
								 UInt16						*recsRotated )
{
	...
	*recordFit		= true;
	...
	{
		printf( "\n%s - bad key order in left node num %d: \n", __FUNCTION__ , SWAP_BE32(rightNode->bLink) );
	...
	{
		printf( "\n%s - bad key order in left node num %d: \n", __FUNCTION__ , SWAP_BE32(leftNode->fLink) );
								 ...
								 UInt16						*recsRotated )
{
	...
	PanicIf ( SWAP_BE32(right->bLink) != 0 && left == 0, "\p SplitLeft: left sibling missing!?" );
	...
	if ( left != nil )
	{
		if ( SWAP_BE32(left->fLink) != rightNodeNum )
	...
	if ( left != nil )
	{
		left->fLink	= SWAP_BE32(newNodeNum);
	...
	left		= leftNode->buffer;
	left->fLink	= SWAP_BE32(rightNodeNum);
	...
	right->bLink		= SWAP_BE32(newNodeNum);			// update Right bLink

	if ( (left->kind == kBTLeafNode) && (SWAP_BE32(left->bLink) == 0) )
	{
								 ...
								 UInt16						*recsRotatedPtr )
{
	...
	PanicIf ( SWAP_BE32(leftPtr->fLink) != 0 && rightPtr == 0, "\p SplitRight: right sibling missing!?" );
	...
	if ( rightPtr != nil )
	{
		rightPtr->bLink	= SWAP_BE32(newNodeNum);
	...
	rightPtr			= rightNodePtr->buffer;
	rightPtr->bLink		= SWAP_BE32(nodeNum);
	...
	leftPtr->fLink		= SWAP_BE32(newNodeNum);			// update Left fLink
								 ...
								 UInt16						*recsRotatedPtr )
{
	...
	*insertIndexPtr = myInsertIndex;
	...
	{
		printf( "\n%s - bad key order in right node num %d: \n", __FUNCTION__ , SWAP_BE32(leftNodePtr->fLink));
	...
	{
		printf( "\n%s - bad key order in left node num %d: \n", __FUNCTION__ , SWAP_BE32(rightNodePtr->bLink));
	...
	if ( DoKeyCheckAcrossNodes( leftNodePtr, rightNodePtr, btreePtr, false ) != noErr )
	{
		printf( "\n%s - bad key order across nodes left %d right %d: \n", 
			__FUNCTION__ , SWAP_BE32(rightNodePtr->bLink), SWAP_BE32(leftNodePtr->fLink) );
		PrintNodeDescriptor( leftNodePtr );
		PrintNodeDescriptor( rightNodePtr );
		err = fsBTBadRotateErr;
		goto ErrorExit;
									...
									Boolean printKeys ) 
{
	...
	GetRecordByIndex( theTreePtr, theLeftNodePtr, (SWAP_BE16(theLeftNodePtr->numRecords) - 1), 
...
static void PrintNodeDescriptor( NodeDescPtr  thePtr )
{
	printf( "    fLink %d bLink %d kind %d height %d numRecords %d \n",
			SWAP_BE32(thePtr->fLink), SWAP_BE32(thePtr->bLink), thePtr->kind, thePtr->height, SWAP_BE16(thePtr->numRecords) );
}
