...
#ifdef __FreeBSD__
	if (ioctl(driveRefNum, DIOCGMEDIASIZE, &devBlockCount) < 0) {
		printf("ioctl(DIOCGMEDIASIZE) for fd %d: %s\n", driveRefNum, strerror(errno));
		return (-1);
	}

	if (ioctl(driveRefNum, DIOCGSECTORSIZE, &devBlockSize) < 0) {
		printf("ioctl(DIOCGSECTORSIZE) for fd %d: %s\n", driveRefNum, strerror(errno));
		return (-1);
	}

	devBlockCount /= devBlockSize;
#else /* !__FreeBSD__ */
