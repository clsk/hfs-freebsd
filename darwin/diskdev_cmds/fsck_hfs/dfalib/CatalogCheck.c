...
CheckDirectory(const HFSPlusCatalogKey * key, const HFSPlusCatalogFolder * dir)
{
	...
	CheckCatalogName(SWAP_BE16(key->nodeName.length), &key->nodeName.unicode[0], SWAP_BE32(key->parentID), false);
...
CheckThread(const HFSPlusCatalogKey * key, const HFSPlusCatalogThread * thread)
{
	...
	result = CheckCatalogName(SWAP_BE16(thread->nodeName.length), &thread->nodeName.unicode[0],
...
CheckFile_HFS(const HFSCatalogKey * key, const HFSCatalogFile * file)
{
	...
	if (SWAP_BE16(file->flags) & kHFSThreadExistsMask)
	 ...
	 * appear to set the bit.  Therefore, we ignore it.
	 */
	if ((SWAP_BE16(file->flags) & (UInt8) ~(0X83)) ||
	...
	if (PtrAndHand(&fileID, (Handle)gScavGlobals->validFilesList, sizeof(UInt32) ) )
		return (R_NoMem);
#endif
	CheckCatalogName_HFS(key->nodeName[0], &key->nodeName[1], SWAP_BE32(key->parentID), false);
