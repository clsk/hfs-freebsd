					...
					UInt32 hint, SInt16 valenceDelta)
{
	...
	if (vcb->vcbSignature == kHFSPlusSigWord) {
		...
		tempData.hfsPlusFolder.valence = SWAP_BE32(SWAP_BE32(tempData.hfsPlusFolder.valence) + valenceDelta);		// adjust valence
		tempData.hfsPlusFolder.contentModDate = SWAP_BE32(timeStamp);	// set date/time last modified
		folderID = SWAP_BE32(tempData.hfsPlusFolder.folderID);
	} else /* kHFSSigWord */ {
		tempData.hfsFolder.valence = SWAP_BE16(SWAP_BE16(tempData.hfsFolder.valence) + valenceDelta);				// adjust valence
		tempData.hfsFolder.modifyDate = SWAP_BE32(GetTimeLocal(true));		// set date/time last modified
...
DeleteCatalogNode(SVCB *vcb, UInt32 pid, const CatalogName * name, UInt32 hint)
{
	...
	nodeParentID = SWAP_BE32(isHFSPlus ? keyp->hfsPlus.parentID : keyp->hfs.parentID);
	...
	case kHFSFileRecord:
		if (SWAP_BE16(rec.hfsFile.flags) & kHFSThreadExistsMask)
