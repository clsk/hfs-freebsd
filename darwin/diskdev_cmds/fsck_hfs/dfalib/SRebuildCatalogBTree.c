        						...
        						UInt32 *			theMapNodeCountPtr 	)
{
	...
	*theBytesUsedPtr = 
	...
	myNodeDescPtr->numRecords = SWAP_BE16(0);
	myOffset = sizeof( BTNodeDescriptor );
							...
							UInt32 				theNodeCount )
{
	...
	for ( i = 0; i < theNodeCount; i++ ) 
	{
		...
		if ( (i + 1) < theNodeCount )
			...
			myNodeDescPtr->fLink = SWAP_BE32(0);  /* this is the last map node */
...
static OSErr DeleteCatalogBTree( SGlobPtr theSGlobPtr, SFCB * theFCBPtr )
{
	...
	if ( isHFSPlus )
	{
		for ( i = 0; i < kHFSPlusExtentDensity; ++i ) 
		{
			...
			theFCBPtr->fcbExtents32[ i ].startBlock = SWAP_BE32(0);
			theFCBPtr->fcbExtents32[ i ].blockCount = SWAP_BE32(0);
		}
	}
	else
	{
		...
		{
			if ( SWAP_BE32(theFCBPtr->fcbExtents16[ i ].blockCount) == 0 )
			{
				checkExtentsBTree = false;
				break;
			}
			(void) BlockDeallocate( myVCBPtr, 
									SWAP_BE32(theFCBPtr->fcbExtents16[ i ].startBlock), 
									SWAP_BE32(theFCBPtr->fcbExtents16[ i ].blockCount) );
			theFCBPtr->fcbExtents16[ i ].startBlock = SWAP_BE32(0);
			theFCBPtr->fcbExtents16[ i ].blockCount = SWAP_BE32(0);
		}
	}
											...
											SFCB * theFCBPtr )
{
	...
	for ( ;; )
	{
		...
		else
		{
			...
			for ( i = 0; i < kHFSExtentDensity; ++i ) 
			{
				if ( SWAP_BE32(myExtentRecord.hfs[ i ].blockCount) == 0 )
					break;

				(void) BlockDeallocate( myVCBPtr, 
										SWAP_BE32(myExtentRecord.hfs[ i ].startBlock),
										SWAP_BE32(myExtentRecord.hfs[ i ].blockCount) );
			}
		}
...
static void PrintNodeDescriptor( NodeDescPtr thePtr )
{
	...
	printf( "   fLink %d \n", SWAP_BE32(thePtr->fLink) );
	printf( "   bLink %d \n", SWAP_BE32(thePtr->bLink) );
...
static void PrintBTHeaderRec( BTHeaderRec * thePtr )
{
	...
	printf( "   treeDepth %d \n", SWAP_BE16(thePtr->treeDepth) );
	printf( "   rootNode %d \n", SWAP_BE32(thePtr->rootNode) );
	printf( "   leafRecords %d \n", SWAP_BE32(thePtr->leafRecords) );
	printf( "   firstLeafNode %d \n", SWAP_BE32(thePtr->firstLeafNode) );
	printf( "   lastLeafNode %d \n", SWAP_BE32(thePtr->lastLeafNode) );
	printf( "   nodeSize %d \n", SWAP_BE16(thePtr->nodeSize) );
	printf( "   maxKeyLength %d \n", SWAP_BE16(thePtr->maxKeyLength) );
	printf( "   totalNodes %d \n", SWAP_BE32(thePtr->totalNodes) );
	printf( "   freeNodes %d \n", SWAP_BE32(thePtr->freeNodes) );
	printf( "   clumpSize %d \n", SWAP_BE32(thePtr->clumpSize) );
	printf( "   btreeType 0x%02X \n", thePtr->btreeType );
	printf( "   attributes 0x%02X \n", SWAP_BE32(thePtr->attributes) );
...
static void PrintLeafNodeRec( HFSPlusCatalogFolder * thePtr )
{
	...
	printf( "   recordType %d ", SWAP_BE16(thePtr->recordType) );
	if ( SWAP_BE16(thePtr->recordType) == kHFSPlusFolderRecord )
		printf( "%s \n", "kHFSPlusFolderRecord" );
	else if ( SWAP_BE16(thePtr->recordType) == kHFSPlusFileRecord )
		printf( "%s \n", "kHFSPlusFileRecord" );
	else if ( SWAP_BE16(thePtr->recordType) == kHFSPlusFolderThreadRecord )
		printf( "%s \n", "kHFSPlusFolderThreadRecord" );
	else if ( SWAP_BE16(thePtr->recordType) == kHFSPlusFileThreadRecord )
