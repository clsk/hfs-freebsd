	...
	UInt32				*foundHint)
{
	...
	if (vcb->vcbSignature == kHFSSigWord) {
		...
		key.fileID		= SWAP_BE32(fileID);
		key.startBlock	= SWAP_BE16((UInt16)startBlock);
		...
		if (err == noErr) {
			...
			foundKey->keyLength 	= SWAP_BE16(kHFSPlusExtentKeyMaximumLength);
			foundKey->forkType		= extentKey.forkType;
			foundKey->pad			= 0;
			foundKey->fileID		= extentKey.fileID;
			foundKey->startBlock	= SWAP_BE32(SWAP_BE16(extentKey.startBlock));
			
			//	Copy the found data back for the caller
			foundData[0].startBlock = SWAP_BE32(SWAP_BE16(extentData[0].startBlock));
			foundData[0].blockCount = SWAP_BE32(SWAP_BE16(extentData[0].blockCount));
			foundData[1].startBlock = SWAP_BE32(SWAP_BE16(extentData[1].startBlock));
			foundData[1].blockCount = SWAP_BE32(SWAP_BE16(extentData[1].blockCount));
			foundData[2].startBlock = SWAP_BE32(SWAP_BE16(extentData[2].startBlock));
			foundData[2].blockCount = SWAP_BE32(SWAP_BE16(extentData[2].blockCount));
			
			for (i = 3; i < kHFSPlusExtentDensity; ++i)
			{
				foundData[i].startBlock = SWAP_BE32(0);
				foundData[i].blockCount = SWAP_BE32(0);
			}
		}
	}
		...
		key.keyLength	= SWAP_BE16(kHFSPlusExtentKeyMaximumLength);
		key.forkType	= forkType;
		key.pad			= 0;
		key.fileID		= SWAP_BE32(fileID);
		key.startBlock	= SWAP_BE32(startBlock);
	...
	UInt32				*hint)
{
	...
	if (vcb->vcbSignature == kHFSSigWord) {
		...
		hfsKey.startBlock = SWAP_BE16(SWAP_BE32(key->startBlock));
	...
	if (SWAP_BE16(fileRec->recordType) == kHFSFileRecord) {
		...
		for (i = 0; i < kHFSExtentDensity; ++i) {
			dataForkExtents[i].startBlock =
				SWAP_BE32(SWAP_BE16(fileRec->hfsFile.dataExtents[i].startBlock));
			dataForkExtents[i].blockCount =
				SWAP_BE32(SWAP_BE16(fileRec->hfsFile.dataExtents[i].blockCount));

			rsrcForkExtents[i].startBlock =
				SWAP_BE32(SWAP_BE16(fileRec->hfsFile.rsrcExtents[i].startBlock));
			rsrcForkExtents[i].blockCount =
				SWAP_BE32(SWAP_BE16(fileRec->hfsFile.rsrcExtents[i].blockCount));
		}
	...
	do {
		...
		if (err == noErr) {
			...
			if (actualStartBlock == startBlock) {
				...
				foundData[foundIndex].blockCount = SWAP_BE32(SWAP_BE32(foundData[foundIndex].blockCount) + actualNumBlocks);
				...
				if (foundIndex == numExtentsPerRecord) {
					...
					foundKey.pad = 0;
					foundKey.fileID = SWAP_BE32(fcb->fcbFileID);
					foundKey.startBlock = SWAP_BE32(nextBlock);
					
					foundData[0].startBlock = SWAP_BE32(actualStartBlock);
					foundData[0].blockCount = SWAP_BE32(actualNumBlocks);
					...
					foundData[foundIndex].startBlock = SWAP_BE32(actualStartBlock);
					foundData[foundIndex].blockCount = SWAP_BE32(actualNumBlocks);
					err = UpdateExtentRecord(vcb, fcb, &foundKey, foundData, hint);
					if (err != noErr) break;
				}
			}
	...
	Boolean			truncateToExtent)	// if true, truncate to end of extent containing newPEOF
{
	...
	if (eofSectors == 0) {
		...
		if (err == noErr) {
			for (i=0; i < numExtentsPerRecord; i++) {
				extentRecord[i].startBlock = SWAP_BE32(0);
				extentRecord[i].blockCount = SWAP_BE32(0);
			}
		}
	...
	if (!truncateToExtent) {
		...
		if (numBlocks != 0) {
			...
			startBlock = SWAP_BE32(extentRecord[extentIndex].startBlock) + SWAP_BE32(extentRecord[extentIndex].blockCount) - numBlocks;
			//	Free the blocks in bitmap
			err = BlockDeallocate(vcb, startBlock, numBlocks);
			if (err != noErr) goto ErrorExit;
			//	Adjust length of this extent
			extentRecord[extentIndex].blockCount = SWAP_BE32(SWAP_BE32(extentRecord[extentIndex].blockCount) - numBlocks);
			//	If extent is empty, set start block to 0
			if (SWAP_BE32(extentRecord[extentIndex].blockCount) == 0)
				extentRecord[extentIndex].startBlock = SWAP_BE32(0);
	...
	//
	while (extentIndex < numExtentsPerRecord && SWAP_BE32(extentRecord[extentIndex].blockCount) != 0) {
		numBlocks = SWAP_BE32(extentRecord[extentIndex].blockCount);
		//	Deallocate this extent
		err = BlockDeallocate(vcb, SWAP_BE32(extentRecord[extentIndex].startBlock), numBlocks);
		if (err != noErr) goto ErrorExit;
		//	Update next file allocation block number
		nextBlock += numBlocks;
		//	Zero out start and length of this extent to delete it from record
		extentRecord[extentIndex].startBlock = SWAP_BE32(0);
		extentRecord[extentIndex].blockCount = SWAP_BE32(0);
	...
	UInt32				*endingFABNPlusOne )
{
	...
	if( err == noErr ) {
		...
		*extentBTreeHint = kNoHint;			// no hint, because not in the BTree
		foundExtentKey->keyLength = SWAP_BE16(0);		// 0 = the FCB itself
	...
	if (err == btNotFound) {
		...
		*extentBTreeHint = kNoHint;
		foundExtentKey->keyLength = SWAP_BE16(0);
		err = GetFCBExtentRecord(vcb, fcb, foundExtentData);
	...
	UInt32					extentBTreeHint)
{
	...
	if (SWAP_BE16(extentFileKey->keyLength) == 0) {	// keyLength == 0 means the FCB's extent record
		...
		if (vcb->vcbSignature == kHFSSigWord) {
			...
			key.startBlock	= SWAP_BE16(SWAP_BE32(extentFileKey->startBlock));
	...
	HFSExtentRecord		newExtents)
{
	...
	// copy the first 3 extents
	newExtents[0].startBlock = SWAP_BE16(SWAP_BE32(oldExtents[0].startBlock));
	newExtents[0].blockCount = SWAP_BE16(SWAP_BE32(oldExtents[0].blockCount));
	newExtents[1].startBlock = SWAP_BE16(SWAP_BE32(oldExtents[1].startBlock));
	newExtents[1].blockCount = SWAP_BE16(SWAP_BE32(oldExtents[1].blockCount));
	newExtents[2].startBlock = SWAP_BE16(SWAP_BE32(oldExtents[2].startBlock));
	newExtents[2].blockCount = SWAP_BE16(SWAP_BE32(oldExtents[2].blockCount));

	#if DEBUG_BUILD
		if (SWAP_BE16(oldExtents[3].startBlock) || SWAP_BE16(oldExtents[3].blockCount)) {
