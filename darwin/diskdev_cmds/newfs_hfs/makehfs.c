...
#ifdef __FreeBSD__
	/*
	 * Just make a Pascal string out of the C one
	 * representing the volume name.
	 */
	if (strlen(defaults->volumeName) <= sizeof(mdbp->drVN) - 1) {
...
#ifdef __FreeBSD__
	/* XXX need to check if the UCS-2 string fits into nodeName */
	utf_decodestr(dp->volumeName, strlen(dp->volumeName), ckp->nodeName.unicode, &len);
	ckp->nodeName.length = SWAP_BE16 ((UInt16) len);
#else /* !__FreeBSD__ */
